         ...
         //  subelement.addComment("H3C注释");
            for(Iterator it = subelement.elementIterator("TrapCodeDef");it.hasNext();){
            	Element its = (Element)it.next();
            	 ...
            	 if(masterSlive!=null){
            		 its.remove(masterSlive);
            		 Element tempMaster = its.element("masterSlive");
            		 if(tempMaster!=null){
            			 System.out.println(its.element("name").getText());
            		 }
            	 } else {
            		 System.out.println(its.element("name").getText()+"   没有master");
            	 }
//           	 if(masterSlive==null)
//           	 {           		 
//           		 System.out.println(its.element("name").getText());
...
//     
//        FileOutputStream output = new FileOutputStream("d:/c18000.xml");
